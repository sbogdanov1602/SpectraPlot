...
void CsvData::LoadSettings()
{
    auto settingsFile = QDir::currentPath() + "/csv_data.ini";
...
int CsvData::Load(std::string inFilePath, std::function<void(int)>  setProgressDlgValue, std::function<void(int)>  setMaximum, bool loadOneFileOnly)
{
    ClearData();
    ...
    if (!loadOneFileOnly) {
        ...
        if (pos >= 0) {
            dir = fname.left(pos);
            QDir directory(dir);
            fileList = directory.entryList(QStringList() << "*.csv" << "*.CSV", QDir::Files);
...
float CsvData::PointScale()
{
    return m_fPointScale;
}

float CsvData::MeasurementStep()
{
    return m_fMeasurementStep;
}

double CsvData::SignalCoeff()
{
    return m_dSignalCoeff;//0.0000625;// 4.096 / 65536 ;
}

std::string CsvData::MapXAxisLabel()
{
    return m_sMapXAxisLabel;//"t (ms)";
}

std::string CsvData::MapYAxisLabel()
{
    return m_sMapYAxisLabel;// "t (sec)";
}

std::string CsvData::SignalAxisLabel()
{
